...
class _DetailsState extends State<Details> {
  ...
  _buildMoreBook(DetailsProvider provider) {
    if (provider.loading) {
      ...
      return ListView.builder(
        ...
        itemCount: provider.related.feed.entries.length,
        itemBuilder: (BuildContext context, int index) {
          Entry entry = provider.related.feed.entries[index];
          return Padding(
  ...
  openBook(DetailsProvider provider) async {
    ...
    if (dlList.isNotEmpty) {
      ...
      List locators = await LocatorDB().getLocator(widget.entry.id.toString());
      ...
      EpubViewer.open(path, lastLocation: locators.isNotEmpty ? EpubLocator.fromJson(locators[0]) : null);
  ...
  _share() {
    Share.text(
      '${widget.entry.title} by ${widget.entry.author}',
      'Read/Download ${widget.entry.title} from ${widget.entry.link}.',
      'text/plain',
    );
  }
}
